...
const app = createGateway({ store, telegramSecretToken: env.telegramSecretToken, githubWebhookSecret: env.githubWebhookSecret, telegramAllowedUserIds: new Set(env.telegramAllowedUserIds) });
...
createServer(async (request, response) => {
  ...
  if (chunks.length > 0 && request.method !== "GET" && request.method !== "HEAD") init.body = Buffer.concat(chunks);
  const result = await app.fetch(new Request(url, init));
  response.statusCode = result.status;
  result.headers.forEach((value, key) => response.setHeader(key, value));
  response.end(Buffer.from(await result.arrayBuffer()));
}).listen(port, "127.0.0.1", () => console.log(`JACK-K gateway listening on 127.0.0.1:${port}`));
