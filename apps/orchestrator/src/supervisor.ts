...
export class Supervisor {
  ...
  public async processApprovals(): Promise<void> {
    ...
    for (const ticket of this.options.store.listTickets({
      ...
      if (
        this.options.store.hasApproval(ticket.id, "reject") ||
        this.options.store.hasApproval(ticket.id, "request_changes")
      ) {
        this.options.store.transition(
          ticket.id,
          "rejected",
          "Approval rejected or changes requested",
        );
      ...
      const completed = this.options.store.getCompletedExecution(ticket.id);
      if (!completed) {
        await this.options.notify(
          ticket.id,
          "Approval found, but the executor result is unavailable after restart; re-run the ticket.",
        );
        continue;
      }
      try {
        await this.options.gitWorker.commit(
          ticket.id,
          `forge: ${ticket.description.slice(0, 72)}`,
        );
        await this.options.gitWorker.push(ticket.id);
        const url = await this.options.gitWorker.openDraftPr(
          ticket.id,
          this.prBody(ticket, completed.result),
        );
        ...
        await this.options.notify(
          ticket.id,
          `Draft PR failed: ${errorMessage(error)}`,
        );
      } finally {
        await this.safeCleanup(ticket.id);
        this.options.store.release(ticket.id);
    ...
    for (const ticket of this.options.store.listTickets({
      ...
      try {
        ...
        this.options.store.transition(
          ticket.id,
          "failed",
          "Supervisor restarted during executor execution",
        );
        this.options.store.release(ticket.id);
      }
    }
  ...
  private async processTicket(ticket: Ticket): Promise<void> {
    try {
      const repo = this.options.repos.find(
        (candidate) => candidate.name === ticket.repo,
      );
      if (!repo)
        throw new Error(`Repository is not allowlisted: ${ticket.repo}`);
      ...
      const authority =
        this.options.mode === "analysis"
          ? AuthorityLevel.L0
          : (Math.min(ticket.authority, repo.authority) as Ticket["authority"]);
      const executor =
        typeof this.options.executor === "function"
          ? this.options.executor(repo)
      ...
      this.options.store.recordCompletedExecution(
        ticket.id,
        prepared.branch,
        result,
      );
      ...
      await this.options.notify(
        ticket.id,
        `Ticket failed: ${errorMessage(error)}`,
      );
    }
  }
  ...
  private async safeCleanup(ticketId: string): Promise<void> {
    try {
      await this.options.gitWorker.cleanup(ticketId);
    } catch {
      /* cleanup is best-effort after a terminal transition */
    }
  }
}

export const createSupervisor = (
  options: Omit<SupervisorOptions, "gitWorker"> & { gitWorker: GitWorker },
